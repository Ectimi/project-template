...
const config: IConfig = {
  ...
  prefix: 'Bookmark_1.0.0', // 名称前缀 建议：项目名 + 项目版本
  expire: 60 * 60 * 24 * 30, //过期时间 单位：秒
  isEncrypt: process.env.NODE_ENV === 'development' ? false : true, // 默认加密 为了调试方便, 开发过程中可以不加密
};
...
// 设置 setStorage
export const setStorage = (key: TKey, value: any, expire = 0) => {
...
// 获取 getStorage
export const getStorage = (key: TKey) => {
...
// 是否存在 hasStorage
export const hasStorage = (key: TKey) => {
  key = autoAddPrefix(key);
...
// 名称前自动添加前缀
const autoAddPrefix = (key: TKey) => {
  const prefix = config.prefix ? config.prefix + '_' : '';
  return prefix + key;
};

// 移除已添加的前缀
const autoRemovePrefix = (key: TKey) => {
